...
scaleX = Math.floor(innerWidth / canvas.width);
scaleY = Math.floor(innerHeight / canvas.height);
...
function touching(object) {
  ...
  rect2 = object;
  if (rect1.radius) {
    rect1.width = rect1.radius;
    rect1.height = rect1.radius;
  }
  if (rect2.radius) {
    rect2.width = rect2.radius;
    rect2.height = rect2.radius;
  }
...
main = function () {
  ...
  for (let i = 0; i < drawQueue.length; i++) {
    ...
    ctx.rotate((Math.PI / 180) * d.angle);
    ctx.rotate(0);
    if (d.type == "rect") {
